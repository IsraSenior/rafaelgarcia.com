...
<template>
    <div class="relative overflow-hidden bg-white rounded-4xl drop-shadow-xl drop-shadow-primary/5 isolate">
        <img class="object-cover object-center w-full aspect-square h-72" :src="service?.image":alt="service?.title" />
        <div class="flex flex-col items-end justify-between p-10">
            ...
            <p>{{ service?.intro }}</p>
            <div>
                <NuxtLink :to="`/servicios/${service?.to}`" class="flex items-center justify-start mt-10 link primary">
                    <span>Leer más</span>
